...
struct TransportTests {
  private func makeTransport(maxRetries: Int = 3) -> Transport {
    ...
    return Transport(config: config, session: MockURLProtocol.makeSession())
  }
  ...
  func `200 OK returns decoded response and caches device/session IDs`() async throws {
    ...
    let body = TrackEnvelope.track(TrackPayload(name: "e"))
  ...
  func `request includes auth and SDK headers`() async throws {
    ...
    let config = try OpenPanel.Config(
      clientId: "00000000-0000-0000-0000-000000000000",
      clientSecret: "secret123",
      apiURL: #require(URL(string: "https://api.example.test")),
      sdkName: "swift",
      sdkVersion: "9.9.9",
      initialRetryDelay: .milliseconds(1)
    )
    let transport = Transport(config: config, session: MockURLProtocol.makeSession())

    let _: TrackResponse? = try await transport.post(path: "/track", body: TrackEnvelope.track(TrackPayload(name: "e")))
    ...
    #expect(request.value(forHTTPHeaderField: "openpanel-client-secret") == "secret123")
    #expect(request.value(forHTTPHeaderField: "openpanel-sdk-name") == "swift")
    #expect(request.value(forHTTPHeaderField: "openpanel-sdk-version") == "9.9.9")
    #expect(request.value(forHTTPHeaderField: "Content-Type") == "application/json")
  }

  @Test
  func `secret header is omitted when clientSecret is nil`() async throws {
    await MockURLProtocol.install { _ in .success(.ok()) }
    ...
    let captured = await MockURLProtocol.registry.requests
    let request = try #require(captured.first)
    #expect(request.value(forHTTPHeaderField: "openpanel-client-secret") == nil)
  }
  ...
  func `202 returns nil with no body`() async throws {
    ...
    let transport = makeTransport()
    let response: TrackResponse? = try await transport.post(path: "/track", body: TrackEnvelope.track(TrackPayload(name: "e")))
    #expect(response == nil)
  }
  ...
  func `POST body is the encoded envelope`() async throws {
    ...
    let transport = makeTransport()
    let body = TrackEnvelope.track(TrackPayload(name: "screen_view", profileId: "u1"))
